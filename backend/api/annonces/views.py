...
class ImageAPIView(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        print(queryset)
        queryset = ImageAnnonce.objects.filter(id_annonce=int(self.kwargs['id_annonce']))[int(self.kwargs['id'])-1].image_url
        data = queryset
        print("image fetched ")
...
class AnnonceRechercheAPIView(generics.RetrieveAPIView):
    ...
    def get(self,serializer):
        ...
        if (searchInfo['wilaya']!=''):
            ...
            print("welcome in no wilyas et communes")
            print(f" la resultat des recherche {queryset}")
            if searchInfo["date_debut"]!='':
                  if searchInfo["date_fin"]!='' :
                      print("first")
        ...
        final_data=[]
        print("\n")
        print(f" la resultat des recherche finale sont : {queryset}")
        print("\n")
...
class UploadAnnoncesAPIView(generics.RetrieveAPIView):
    ...
    def get(self,serializer):
        ...
        return Response(data)
############################################
class MesAnnoncesAPIView(generics.ListCreateAPIView):
    def get(self,serializer):
        queryset = Annonce.objects.filter(id_utilisateur=self.request.user)
        print(queryset)
        data =ResultatAnnonceSerializer(queryset,many=True).data
        print("\n\n")
        print(data)
        print("\n\n")
        return Response(data)